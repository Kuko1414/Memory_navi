...
### ★65% 基线 vs 现在（用户问，git 取证；定性版，详见记忆 [[explore-65pct-baseline-vs-current]]）
- **哪版到 65%**：提交 **`2fe4994`**（"explore mode: Qwen-authored stabilization"）的**纯 Qwen 精简版 explore(702 行)**，
  召回 **13/20=65%**（GT observable=20），`test_explore_stable` 9 项绿。
- **为什么能到**：**放开记录**——纯 Qwen 作者(inspect 出物体+ROI)+代码 depth 回填+去重整理，**没有**后来的预过滤门；
  Qwen 报什么基本都留 → 召回高(幻觉多但不进召回分母)。C1 强制规范中文单数名(跨帧一致)+名感知去重+门校验。
- **这版(阶段十三/十四)有什么不同**：加了 ①混合标注(YOLO ROI+Qwen 命名弃权门+全图 merge) ②均匀网格覆盖(取代 frontier/调度官)
  ③**曾加的三道预过滤门**(下一提交 `23ee29d`)：LOS 穿墙反证 + ROI 过小门 + FAR_LABEL_M 距离门。
- **为什么召回一度低于 65%（已定位）**：**主因=过度预过滤**——v6c 实测 **LOS 穿墙反证丢 49 + ROI 过小门丢 24**，
  **大量是贴墙/稍远真家具被误杀**（FAR_LABEL_M 在好覆盖下丢 0、非主因，我先前误判它是最大嫌疑，此处订正）。
  次因：相机 0.13m(桌被显示器盖过等，65%版也有但那次样本高) + 单轮方差 + 大房间 nav_cap + 跨轮车漂移。
- **能否回 65%**：**能**——本会话关掉 LOS+ROI 门(`FILTER_ROI_LOS=0`,已默认关) → **v7=59%(10/17)逼近**；
  配合均匀网格(覆盖比 65%版强)+多轮取稳定值应能到/超。真正没变的天花板=相机高度。**教训：`23ee29d` 那批"精度/防幻觉"预过滤用召回换精度、过度了。**
